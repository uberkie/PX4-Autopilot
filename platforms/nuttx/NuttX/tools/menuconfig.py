...
def _load_config():
    ...
    return any(
        ...
        and sym.type in (BOOL, TRISTATE)
        and sym.tri_value != sym.user_value
        or sym.user_value is not None
        and sym.type not in (BOOL, TRISTATE)
    ...
    while True:
        ...
        if c == "y":
            if _try_save(_kconf.write_config, standard_config_filename(),
                ...
                return f"Configuration saved to '{standard_config_filename()}'"
...
def _enter_menu(menu):
    ...
    if shown_sub := _shown_nodes(menu):
...
def _change_node(node):
    ...
    if sc.type in (INT, HEX, STRING):
        ...
        while True:
            s = _input_dialog(
                f"{node.prompt[0]} ({TYPE_TO_STR[sc.type]})",
                s,
                _range_info(sc),
            )
...
def _save_dialog(save_fn, default_filename, description):
    ...
    while True:
        ...
        if _try_save(save_fn, filename, description):
            _msg("Success", f"{description} saved to {filename}")
            return True
...
def _kconfig_def_info(item):
    ...
    s = f'Kconfig definition{"s" if len(nodes) > 1 else ""}, with propagated dependencies\n'
    s += (len(s) - 1)*"="
...
def _include_path_info(node):
    return (
        ...
        else f'Included via {" -> ".join(f"{filename}:{linenr}" for filename, linenr in node.include_path)}\n'
    )
...
def _check_validity(sym, s):
    ...
    try:
        ...
        _error(f"'{s}' is a malformed {TYPE_TO_STR[sym.type]} value")
        return False
    ...
    for low_sym, high_sym, cond in sym.ranges:
        if expr_value(cond):
            ...
            if not low <= val <= high:
                _error(f"{s} is outside the range {low_sym.str_value}-{high_sym.str_value}")
