...
class PubSub(object):
    ...
    def __init__(self, name, topic_blacklist, regexes):
        ...
        self._regexes = {re.compile(regex) for regex in regexes}
        ...
        for regex in self._regexes:
            ...
            if route_group:
                if route_group == 'ORB_ID':
                    log.debug(
                        f"            >>> Found ORB_ID topic: {topic_group}    w/regex: {str(regex.pattern)}"
                    )
                                ...
                                (
                                    (
                                        (
                                            (
                                                (
                                                    f'!!! Encountered regex case:  `route_group` contains unrecognized value!: {route_group}  (::{str(regex.pattern)}'
                                                    + ')\n'
                                                    + "        ("
                                                )
                                                + route_group
                ...
                log.debug(
                    f'!!! Found ambiguous site, without `ORB_ID` or topic (::{str(regex.pattern)}'
                )
                return PubSub.AMBIGUOUS_SITE_TOPIC

            else:
                raise SyntaxError(
                    f"            !!! unhandled case:  unknown-variant: {route_group}, {topic_group} ....from regex: {str(regex.pattern)}"
                )
    ...
    def _filter_topic(self, topic_name: str) -> str:
        ...
        if topic_name not in self._topic_blacklist:
            return topic_name
        log.debug(f"                    XX Ignoring blacklisted topic {topic_name}")
        return None
...
class Scope(object):
    ...
    def is_empty(self):
        return len(self.publications) == 0 == len(self.subscriptions)
...
class Graph(object):
    ...
    def _get_current_scope(self):
        return None if len(self._current_scope) == 0 else self._current_scope[-1]
    ...
    def build(self, src_path_list, **kwargs):
        ...
        self._path_blacklist = {
            os.path.normpath(p) for p in kwargs.get('path_blacklist', [])
        }

        for path in src_path_list:
            log.info(f"## Add src path: {path}")
        ...
        log.info(f'    Library Count:      {len(self._found_libraries)}')
        log.info(f'    Module Count:       {len(self._found_modules)}')
        log.info(f'    Warning Count:      {len(self._warnings)}')
        ...
        log.info(f'    Scope Count:        {len(self._print_scopes)}')
        log.info(f'    Ambiguous Topics:   {len(self._print_ambiguities)}')
        log.info(f'    Linked Topics:      {len(self._print_topics)}')
    ...
    def _generate_print_lists(self, use_topic_pubsub_union, merge_depends):
        ...
        if len(self._scope_whitelist) == 0:
            select_scopes = self._found_modules
        else:
            select_scopes = {
                scope_name: all_scopes[scope_name]
                for scope_name in self._scope_whitelist
                if scope_name in all_scopes
            }
        if not isinstance(select_scopes, dict) or len(select_scopes) == 0:
        ...
        for name, scope in select_scopes.items():
            ...
            log.debug(f'        ## Subs: {len(scope.subscriptions)}')
            for topic in sorted(scope.subscriptions):
                log.debug(f'            - {topic}')
                subscribed_topics.add(topic)

            log.debug(f'        ## Pubs: {len(scope.publications)}')
            ...
            log.debug(f'        ## Ambiguities: {len(scope.ambiguities)}')
        ...
        if use_topic_pubsub_union:
            ...
            intersect_scopes = {}
            for scope in scopes_with_topic.values():
    ...
    def _build_recursive(self, path, **kwargs):
        if os.path.normpath(path) in self._path_blacklist:
            log.debug(f'ignoring excluded path {path}')
            return
        ...
        for line in datafile:
            if 'px4_add_module' in line: # must contain 'px4_add_module'
                ...
                if len(tokens) > 1:
                    ...
                    if self._in_scope():
                        log.debug(f'    >> found library: {library_name}')
                ...
                if line.startswith('\t\t') and not line.strip().startswith('#'):
                    ...
                    for name in depends:
                        ...
                        if kwargs['merge_depends']:
                            if (
                                len(self._scope_whitelist) > 0
                                and self._current_scope[-1].name
                                in self._scope_whitelist
                            ):
            ...
            if found_module_def and 'MAIN' in words and len(words) >= 2:
                ...
                if self._in_scope():
                    log.debug(f'    >> Found module name: {module_scope.name}')
    ...
    def _process_source_file(self, file_name):
        ...
        log.debug(
            "        >> {:}extracting topics from file: {:}".format(
                f"{current_scope.name}: " if current_scope is not None else "",
                file_name,
            )
        )
        ...
        with codecs.open(file_name, 'r', 'utf-8') as f:
            try:
                ...
                print(f'Failed reading file: {file_name}, skipping content.')
                return


            if current_scope:
                if (
                    current_scope.name in self._scope_blacklist
                    or current_scope.name == 'uorb_tests'
                    or current_scope.name == 'uorb'
                ):
                    return
            line_number = 0
            for full_line in content.splitlines():
                ...
                if topic := self._publications.match(short_line):
                    if not current_scope:
                        raise AssertionError(
                            f"Encountered Publication topic outside of any scope! {file_name} Aborting!"
                        )

                    current_scope.publications.add(topic)
                    continue
                if topic := self._subscriptions.match(short_line):
                    if not current_scope:
                        raise AssertionError(
                            f"Encountered Subscription topic outside of any scope! {file_name} Aborting!"
                        )

                    current_scope.subscriptions.add(topic)
                    continue
                if topic := self._ambiguities.match(short_line):
                    if not current_scope:
                        raise AssertionError(
                            f"Encountered Ambiguous topic outside of any scope! {file_name} Aborting!"
                        )
    ...
    def merge_depends(self):
        ...
        for modname,module in self._found_modules.items():
            if modname in self._scope_whitelist or len(self._scope_whitelist) == 0:
                for depname in module.dependencies:
                    if depname in self._found_libraries:
                        ...
                        for _ in dep.ambiguities:
                            module.ambiguities

        # omit all libraries -- they've already been merged into their respective dependees
        self._scope_whitelist = {
            str(s) for s in self._scope_whitelist if s not in self._found_libraries
        }
        ...
        if show_subscriptions:
            for scope_name,scope in output_scopes.items():
                for topic in scope.subscriptions:
                    if topic in output_topics:
                        graph.edge(f't_{topic}', f'm_{scope_name}', color=topic_colors[topic])
...
class OutputJSON(object):
    ...
    def write(self, file_name):
        ...
        for scope_tuple in sorted(output_scopes.items(), key=(lambda st: len(st[0])), reverse=True):
            node = {
                'id': f'm_{scope_tuple[0]}',
                'name': scope_tuple[0],
                'type': scope_tuple[1].typename,
                'color': '#666666',
            }
        ...
        for topic in sorted(output_topics, key=len, reverse=True):
            node = {
                ...
                'type': 'topic',
                'color': topic_colors[topic],
                'url': f'https://github.com/PX4/PX4-Autopilot/blob/main/msg/{topic_filename(topic)}.msg',
            }
            nodes.append(node)
        ...
        for name, scope in output_scopes.items():
            for topic in scope.subscriptions:
                if topic in output_topics:
                    edge = {'source': f't_{topic}', 'target': f'm_{name}'}
...
if __name__ == "__main__":
    ...
    print(f'Excluded topics: {topic_blacklist}')
    ...
    for path in args.src_path:
        if os.path.exists(path):
            ...
            log.warn(f"Could not find path: {path}")
    ...
    if path_blacklist:
        print(f'Excluded Path: {str(path_blacklist)}')
    ...
    if args.output == 'json':
        ...
        output_json.write(f'{args.file}.json')

    elif args.output in ('graphviz','gv'):
        try:
            from graphviz import Digraph
        except ImportError as e:
            print(f"Failed to import graphviz: {str(e)}")
            print("")
        ...
        output_graphviz.write(f'{args.file}.fv', engine=engine)
        output_graphviz.write(
            f'{args.file}_subs.fv', show_publications=False, engine=engine
        )
        output_graphviz.write(
            f'{args.file}_pubs.fv', show_subscriptions=False, engine=engine
        )
    elif args.output != 'none':
        print(f'Error: unknown output format {args.output}')
